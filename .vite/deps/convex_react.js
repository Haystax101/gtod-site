...
function compareUTF8(a, b) {
  ...
  for (let i2 = 0; i2 < length; ) {
    const aCodePoint = a.codePointAt(i2);
    const bCodePoint = b.codePointAt(i2);
    ...
    i2 += utf16LengthForCodePoint(aCodePoint);
  }
...
function compareSameTypeValues(v1, v2) {
  ...
  for (let i2 = 0; i2 < v1.length && i2 < v2.length; i2++) {
    const cmp = compareAsTuples(v1[i2], v2[i2]);
    if (cmp !== 0) {
      return cmp;
    }
  }
...
function usePaginatedQueryInternal(query, args, options, throwOnError = true) {
  ...
  const [results, maybeLastResult, maybeError] = (0, import_react4.useMemo)(() => {
    ...
    for (const pageKey of currState.pageKeys) {
      ...
      allItems.push(
        ...isIncludingPageKeys ? currResult.page.map((i2) => ({
          ...i2,
